...
def TrainAndTest(alpha_reward, beta_reward, Tf, Nit, discount_factor, num_episodes, epsilon, batch, Channel):
    ...
    TransEnv = Envs.EnvFeedbackGeneral(Tf, alpha_reward, beta_reward, Channel_Local, batch, M=1)
    TransEnv = TransEnv.to(device)
    model_file = 'ModelCNNBatch_Memory'+string_alpha+'.pickle'
    t0 = time.time()
    ...
    with open('Data/AgentCNNRLresultsTestBatch_Memory.pickle', 'ab') as f:
...
with open('Data/AgentCNNRLresultsTestBatch_Memory.pickle', 'wb') as f:
